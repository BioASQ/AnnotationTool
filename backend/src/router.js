...
exports.createRouter = function (model, authentication) {
  ...
  router.path(/\/login\/?/, function () {
      ...
      /*
      * GET to /login with parameters: email and password (code)
      *
      * This is an one-time login to login and change the password. 
      * The password parameter is a random code generated by Login.changePassword.
      */
      this.get().bind(function (req, res, body) {
          if (body.email && body.password) {      
              authentication.oneTimeLogin(body.email, body.password, function (err, result) {
                  if (err) {
                      accountBody.error = err;
                      res.send(500, headers, accountBody);
                  }
  ...
  router.path(/\/remindPassword\/?/, function () {
      /*
      * GET to /remindPassword with parameters: email
      *
      * Generates a temporarily login code and sends an email to the user with a login link.
      */
      this.get().bind(function (req, res, body) {
          if (body.email) {
              var url = 'http://' + req.headers.host + '/login'
              authentication.generateTmpPassword(url, body.email, function (err, result) {
                  if (err) {
                      accountBody.error = err;
                      res.send(500, headers, accountBody);
                  } else if (result) {
